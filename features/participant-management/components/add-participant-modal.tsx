...
export function AddParticipantModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSuccess?.();
      onClose();
      showToast("Add participant successfully");
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && handleMinimize()}>
      <DialogContent
        className="p-0 gap-0 rounded-xl overflow-hidden border-0 shadow-xl [&>button]:hidden w-[calc(100%-32px)] max-w-[460px] md:w-[460px]"
      >
        <button
        ...
        <div className="px-6 md:px-8 pb-4 flex flex-col gap-4 border-t border-[#E2E4E6] pt-5 max-h-[60vh] overflow-y-auto">
          
          {/* Medical Record */}
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center">
              <FieldLabel>Medical Record</FieldLabel>
              <span className={`text-xs ${form.medicalRecordNo.length >= 20 ? 'text-red-500' : 'text-gray-400'}`}>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className={`text-xs ${form.fullName.length >= 100 ? 'text-red-500' : 'text-gray-400'}`}>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className={`text-xs ${form.phone.length >= 16 ? 'text-red-500' : 'text-gray-400'}`}>
        ...
        {/* Tombol aksi dengan gaya warna awal asli */}
